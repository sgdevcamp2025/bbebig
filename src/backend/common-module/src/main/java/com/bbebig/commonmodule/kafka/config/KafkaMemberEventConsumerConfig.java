...
@EnableKafka
@Configuration
public class KafkaMemberEventConsumerConfig {

	@Value("${spring.kafka.bootstrap-servers}")
	private String bootstrapAddress;

	@Value("${spring.kafka.consumer.group-id.member-event}")
	...
	@Bean
	public ConsumerFactory<String, MemberEventDto> consumerFactoryForMemberEvent() {
		return new DefaultKafkaConsumerFactory<>(consumerConfigurations(), new StringDeserializer(),
				new JsonDeserializer<>(MemberEventDto.class));
	}

	@Bean
	public ConcurrentKafkaListenerContainerFactory<String, MemberEventDto> memberEventListener() {
		ConcurrentKafkaListenerContainerFactory<String, MemberEventDto> factory = new ConcurrentKafkaListenerContainerFactory<>();
		factory.setConsumerFactory(consumerFactoryForMemberEvent());
